...
public class DeletedPasswordAdapter extends RecyclerView.Adapter<DeletedPasswordAdapter.ViewHolder> {
    ...
    public DeletedPasswordAdapter(Context c, ArrayList<PasswordData> list) {
        contacts = list;
        this.c = c ;
    }
...
//                        database.deletePassword(contacts.get(holder.getAdapterPosition()).getId(),contacts.get(holder.getAdapterPosition()).getOfUser(),contacts.get(holder.getAdapterPosition()).getURL(),contacts.get(holder.getAdapterPosition()).getUsername(),contacts.get(holder.getAdapterPosition()).getPassword());
                        database.insertPassword(contacts.get(holder.getAdapterPosition()).getOfUser(),contacts.get(holder.getAdapterPosition()).getURL(),contacts.get(holder.getAdapterPosition()).getUsername(),contacts.get(holder.getAdapterPosition()).getPassword());
                        database.close();

                        MainActivity3.contacts.add(MainActivity3.contacts.get(holder.getAdapterPosition()));
                        MainActivity3.deletedpassword.remove(holder.getAdapterPosition());
